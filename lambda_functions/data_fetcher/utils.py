...
def dataframe_to_dict_list(df: pl.DataFrame) -> List[Dict[str, Any]]:
    ...
    for col in df.columns:
        if df[col].dtype in [pl.Datetime, pl.Date]:
            df = df.with_columns(
                pl.col(col).dt.to_string("%Y-%m-%dT%H:%M:%S%.f").alias(col)
            )
...
def transform_weekly_data_to_players(weekly_df: pl.DataFrame, include_all_weeks: bool = False) -> List[Dict[str, Any]]:
    """
    Transform weekly player DataFrame into standardized player format.

    Args:
        weekly_df: DataFrame from nflreadpy.import_weekly_data()
        include_all_weeks: If True, return all weekly records. If False, group by player_id
                          and return only latest week (keeps response < 6MB Lambda limit)
    ...
    if include_all_weeks:
        # Return all weekly records (use with API filtering to stay under 6MB)
        return dataframe_to_dict_list(weekly_df)
    else:
